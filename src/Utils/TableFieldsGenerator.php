...
class TableFieldsGenerator
{
    ...
    public function __construct($tableName, $ignoredFields, $connection = '')
    {
        ...
        $mappings = config('artisanapi.velocity.from_table.doctrine_mappings', []);
        ...
        $this->defaultSearchable = config('artisanapi.velocity.options.tables_searchable_default', false);
    }
        ...
        foreach ($this->columns as $column) {
            ...
            switch ($type) {
                ...
                case 'boolean':
                    ...
                    $field = $this->generateField($column, 'boolean');
                    break;
                case 'datetime':
                    $field = $this->generateField($column, 'datetime');
                    break;
                case 'datetimetz':
                    $field = $this->generateField($column, 'dateTimeTz');
                    break;
                case 'date':
                    $field = $this->generateField($column, 'date');
                    break;
                case 'time':
                    $field = $this->generateField($column, 'time');
                    break;
                case 'decimal':
                ...
                case 'float':
                    ...
                    break;
                case 'string':
                    $field = $this->generateField($column, 'string');
                    break;
                case 'text':
                    $field = $this->generateField($column, 'text');
                    break;
                default:
                    $field = $this->generateField($column, 'string');
                    break;
    ...
    {
        if (!config('artisanapi.velocity.timestamps.enabled', true)) {
            return [];
        }

        $createdAtName = config('artisanapi.velocity.timestamps.created_at', 'created_at');
        $updatedAtName = config('artisanapi.velocity.timestamps.updated_at', 'updated_at');
        $deletedAtName = config('artisanapi.velocity.timestamps.deleted_at', 'deleted_at');
     ...
     * @return GeneratorField
     */
    private function generateField($column, $dbType)
    {
        $field = new GeneratorField();
        $field->name = $column->getName();
